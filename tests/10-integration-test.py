...
class BundleIntegrationTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        if self.bundle:
            self.bundle_path = os.path.abspath(bundle)
        else:
       ...
       try:
           ...
           message = 'Unable to set up the environment in %d seconds.' % seconds
   ...
   def test_1_relations(self):
       """ Test the bundle relations errors will be thrown if relations fail. """
       kube0, kube1, km = self.kunits()
       # Get all the important relations from the bundle. 
       etcd_relation = kube0.relation('etcd', 'etcd:client')
       master_relation = kube0.relation('api', 'kubernetes-master:minions-api')
       minion_relation = km.relation('minions-api', 'kubernetes:api')
       return etcd_relation, master_relation, minion_relation, flannel_relation
